...
let test_digest ctxt =
  ...
  assert_equal ("\x07\xe5\x47\xd9\x58\x6f\x6a\x73\xf7\x3f\xba\xc0\x43\x5e\xd7\x69\x51"^
...
let test_equal ctxt =
  ...
  let h'  = "B" ^ (String.make (Hash.size - 1) 'A') in
  let h'' = (String.make (Hash.size - 1) 'A') ^ "B" in
  assert_bool "=" (Hash.equal (Hash.String.to_hash h)
                              (Hash.String.to_hash h));
  assert_bool "<>" (not (Hash.equal (Hash.String.to_hash h)
                                    (Hash.String.to_hash h')));
  assert_bool "<>" (not (Hash.equal (Hash.String.to_hash h)
                                    (Hash.String.to_hash h'')))
